    ...
    private void handWebsocketFrame(ChannelHandlerContext ctx, WebSocketFrame frame) {
        ...
        try {
            ...
            switch (code) {
                ...
                // TODO 用户答题情况及分数等更新
                case MessageCodeConstant.GAME_READY_MESSAGE_CODE:
                    SessionHolder.channelGroup.writeAndFlush(new TextWebSocketFrame(JSONObject.toJSONString(json)));
                    break;
                default:
            }
...
//        ctx.channel().config().setWriteBufferLowWaterMark();
    ...
    private void handHttpRequest(ChannelHandlerContext ctx, FullHttpRequest request) {
        ...
        if (handshaker == null) {
            ...
            //TODO 若此时在线人数==4,向当前等待列表的4个用户发送消息，表示可以成组
            if (userList.size()==4){
                WsMessage msg2=new WsMessage();
                Map<String, Object> ext2 = new HashMap<String, Object>();
                ext2.put("userList",userList);
                msg2.setExt(ext2); // 用户列表
                msg2.setCode(MessageCodeConstant.GAME_READY_MESSAGE_CODE); //表示可以进行游戏
                SessionHolder.channelGroup.writeAndFlush(new TextWebSocketFrame(JSONObject.toJSONString(msg2)));
            }
